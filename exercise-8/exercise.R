...
print(str(pulitzer)) # all the data circulation values are characters
...
most.pulitzer.prizes.gained <- as.character(pulitzer %>% filter(Pulitzer.Prize.Change == max(Pulitzer.Prize.Change)) %>% select(Newspaper))
...
biggest.decrease.circulation <- as.character(pulitzer %>% filter(Pulitzer.Prize.Winners.and.Finalists..1990.2014 > 4) %>% filter(min(Change.in.Daily.Circulation..2004.2013) == Change.in.Daily.Circulation..2004.2013) %>% select(Newspaper))
...
# Out of the newspapers that decreased in pulitzers, which one had the largest gain in circulation?
biggest.circulation.gain.but.decreased.pulitzers <- as.character(pulitzer %>% filter(Pulitzer.Prize.Change < 0) %>% filter(Change.in.Daily.Circulation..2004.2013 == max(Change.in.Daily.Circulation..2004.2013)) %>% select(Newspaper))